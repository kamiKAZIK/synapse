...
trait NamedObjects {
  ...
  /**
    ...
    * @param objectGenerator a 0-ary function which will generate the NObj if it doesn't already exist.
    ...
    */
  def getOrElseCreate[O <: NamedObject](name: String, objectGenerator: => O)
                                       (implicit timeout: FiniteDuration = defaultTimeout,
                                        persistence: NamedObjectPersistence[O]): O
  ...
  /**
    ...
    * @param objectGenerator a 0-ary function which will be called to generate the object in the caller's thread.
    * @tparam O <: NamedObject the generic type of the object.
    * @return the object with the given name.
    */
  def update[O <: NamedObject](name: String, objectGenerator: => O)
                              (implicit timeout: FiniteDuration = defaultTimeout,
                               persistence: NamedObjectPersistence[O]): O
  ...
  def destroy[O <: NamedObject](objOfType: O, name: String)
                               (implicit persister: NamedObjectPersistence[O]) : Unit
